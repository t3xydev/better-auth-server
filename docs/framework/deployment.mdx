...
4. Confirm `PORT=3000`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, and `DATABASE_URL` (IaC wires Postgres/Redis refs; secrets use `preserve()`).
...
6. Point the public domain at port **3000**. Clear leftover dashboard settings from older templates (Railpack build command, healthcheck `/health`, target port `8080`, app sleeping) — those fight the Dockerfile / IaC settings and fail the deploy.
