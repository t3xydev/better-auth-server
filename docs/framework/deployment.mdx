...
Do not hand-edit generated files (`Dockerfile`, `.dockerignore`, `docker-compose.yml`, `.railway/railway.ts`, `wrangler.jsonc`). Edit the config and re-sync.

| Target | Files | Notes |
|--------|--------|--------|
| **Railway** | `.railway/railway.ts` + `Dockerfile` | Dockerfile builder; healthcheck `/api/health` (2xx liveness) |
...
Railway [Config as Code](https://docs.railway.com/config-as-code) (`railway.toml`) is deprecated. This kit uses [Infrastructure as Code](https://docs.railway.com/infrastructure-as-code) under `.railway/railway.ts`.

1. Install the Railway CLI and `pnpm` dep `railway` (already a devDependency).
2. Link the project: `railway link`.
3. Preview / apply canvas + service settings: `railway config plan` then `railway config apply`.
4. Confirm `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, and `DATABASE_URL` (IaC wires Postgres/Redis refs; secrets use `preserve()`). Do **not** set a custom `PORT` — Railway injects it for healthchecks and public networking.
5. Deploy from `main`. Healthcheck is `/api/health` (always **2xx** when the process is up). For DB readiness probes use `/api/health?ready=1`.
6. Point the public domain at the service (Railway maps the injected `PORT`). Clear leftover dashboard settings from older templates (Railpack build command, healthcheck `/health`, forced port mismatches, app sleeping) — those fight the Dockerfile / IaC settings and fail the deploy.

The Docker runner image must include `source.config.ts`, a writable `.source` directory, and `docs/framework`. fumadocs-mdx recompiles `source.config.mjs` into `.source` on every `next start`; if that directory is root-owned, the process crashes with permission denied.
...
docker run --env-file .env -p 3000:3000 better-auth-server
```
